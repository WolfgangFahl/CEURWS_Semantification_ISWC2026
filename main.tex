...
To make CEUR-WS metadata fair a set of relevant queries should be supported, as derived from the original set of queries of the 2014 Semantic publishing challenge as outlined in Section~\ref{sec:sempub_challenge}.
...
\begin{enumerate}
    \item To provide the tools, infrastructure and approaches to modify the CEUR-WS publication workflow to consistently supply high quality FAIR metadata (Semantification) as detailed in section~\ref{sec:implementations_and_demos}.
    ...
    \item Providing a bootstrapping~\cite{Bardini2001,Engelbart2007} approach to shift the single point of truth from the current HTML/PDF files to a RDF ready ontology based format.

\end{enumerate}

These contributions are valuable for other publishing use cases see section ~\ref{sec:future_work}.

\section{Related Work}\label{sec:Related Work}
%BrylEtAl
\subsection{Semantic Publishing challenge}\label{sec:sempub_challenge}
Representing digital traces of scholarly communication in Knowledge Graphs (KGs) ~\cite{Cousijn2021}, ~\cite{Hogan_2021} is useful for supporting use cases such as literature search and recommendation of events for attendance or publishing. Following the  \tquote{Semantic Web}~\cite{BernersLee2001} approach the Semantification of CEUR-WS has been publicly pursued in the Semantic Publishing challenge~\cite{Lange2014,Sateli2015}. Participants were asked to extract an RDF knowledge graph to be able to answer a set of 20 challenge queries.

Kolchin et al.~\cite{Kolchin2014,Kolchin2015} have submitted an approach using XPath Queries on the HTML DOM markup and converted the results directly to RDF triples; see  \footnotelink{https://github.com/ailabitmo/ceur-ws-lod}{ceur-ws-lod repository on GitHub}.
...
All challenge contributions had a purely scientific focus and therefore they would not have been fit for making the results operational and being used in the actual CEUR-WS publishing workflow, as doing so would have made it mandatory to operate and maintain the necessary local  self-maintained RDF/SPARQL endpoint to provide a knowledge graph as a point of truth.

For lack of resources (human as well as hardware), and limited trust in the long term stability of such approaches the CEUR-WS team did not make the effort.
...
Wikidata is well suited as a FAIR infrastructure.
...
A key goal of metadata first semantic publishing is the early
mandatory use of persistent identifiers for the entities to be linked.

The following PIDs are already often in use:
\begin{itemize}
...
snapshot to the Single Point of Truth. This process ensures the completeness and quality of the metadata
allowing for the generation of  traditional HTML pages and the official stamping as a CEUR-WS  proceeding.
...
and editors early in the life cycle of an event. According to the legal advisors we asked
this is legal since a combination of conditions (Art.\ 6 GDPR) is met:
