...
\end{figure}
Traditional publishing workflows often involve redundant and manual curation of metadata for the most relevant entities (see Figure~\ref{fig:core_entities}) at various stages in the lifecycle of an event, each leaving distinct digital traces. Therefore, the metadata is not readily available for applying state-of-the-art Findable, Accessible, Interoperable, and Reusable (FAIR) ~\ref{sec:FAIR}, Single Point of Truth (SPoT), and Single Source of Truth (SSoT) principles. This work shows how the HTML/PDF SSoT of CEUR-WS at the ceur-ws.org domain (SPoT) has been used to create a Knowledge Graph representation of the relevant entities in Wikidata.
